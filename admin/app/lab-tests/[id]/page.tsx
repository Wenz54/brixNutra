...
/**
 * Edit Lab Parameter Page - Редактирование параметра анализа
 */
...
export default function EditLabParameterPage() {
  ...
  const [parameter, setParameter] = useState<LabParameter | null>(null);
  ...
  const fetchParameter = async () => {
    try {
      ...
      // const parameterData = await api.get<LabParameter>(`${endpoints.labParameters}/${parameterId}`);
      ...
      setParameter(mockParameter);
      
      // Преобразование для формы
      reset({
        parameterId: mockParameter.parameterId,
        name: mockParameter.name,
        category: mockParameter.category,
        units: mockParameter.units.join(', '),
        description: mockParameter.description,
        referenceRanges: mockParameter.referenceRanges,
        lowCauses: mockParameter.lowCauses?.map(c => ({ value: c })) || [{ value: '' }],
        highCauses: mockParameter.highCauses?.map(c => ({ value: c })) || [{ value: '' }],
        recommendations: mockParameter.recommendations || '',
      });
    } catch (err) {
      console.error('Error fetching parameter:', err);
      setError('Не удалось загрузить параметр');
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSubmit = async (data: LabParameterFormData) => {
    try {
      ...
      // TODO: Раскомментировать когда API будет готов
      // await api.put(`${endpoints.labParameters}/${parameterId}`, parameterData);

      console.log('Updating parameter:', parameterData);
      alert('Параметр обновлен! (mock)');
      router.push('/lab-tests');
    } catch (err) {
      console.error('Error updating parameter:', err);
      setError('Не удалось обновить параметр');
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Загрузка параметра...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div>
        ...
        <h1 className="mt-4 text-3xl font-bold text-gray-900">Редактировать параметр</h1>
        <p className="mt-2 text-gray-600">ID: {parameterId}</p>
      </div>
